...
const GetUser = async (req: Request, res: Response) => {
  const { page, limit, attendance_id, class_id, search, role, sortField, sortOrder } = req.query;
  const p = Number(page);
...
const GetUserInfoById = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { _id, email, fullname, username, phone_number, dob, gender, address, avatar, role } = user;
    res.status(200).json(
